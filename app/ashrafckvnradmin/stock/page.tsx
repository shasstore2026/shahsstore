...
export default async function StockDashboard() {
  const [products, shirtStyles] = await Promise.all([
    getProducts(),
    getShirtStyles(),
  ]);
  ...
  const catalogStats = [
    ...
    { label: "Shirt Style / Category", value: shirtStyles.length, color: "bg-purple-700" },
  ];
          ...
          <div className="space-y-2">
            {lowStockProducts.map((p) => {
              ...
              return (
                ...
                >
                  <Link
                    href={`/ashrafckvnradmin/products/${p.id}`}

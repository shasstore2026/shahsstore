    ...
    return (
      <div className="bg-[#FAFAF8] min-h-screen flex items-center justify-center pt-24">
        <div className="text-center px-8">
          <p
            className="text-5xl text-stone-300 font-light mb-6"
            style={{ fontFamily: "'Cormorant Garamond', serif" }}
          >
            Your cart is empty.
          </p>
          <p className="text-stone-400 text-sm font-light mb-8">
            Looks like you haven't added any shirts yet.
          </p>
          <Link
            href="/products"
    ...
    <div className="bg-[#FAFAF8] min-h-screen pt-20 md:pt-32 pb-12 md:pb-24">
      <div className="max-w-5xl mx-auto px-4 md:px-8">
        ...
        <div className="mb-8 md:mb-12">
          <p className="text-xs tracking-[0.4em] text-stone-400 uppercase mb-2">Review</p>
          <h1
          ...
          <div className="lg:col-span-2 space-y-6">
            {cartItems.map((item) => {
              ...
              <div
                ...
                className="flex gap-6 bg-white border border-stone-100 p-5"
              >
                {/* Image */}
                <div className="relative w-24 h-28 bg-stone-100 flex-shrink-0 overflow-hidden">
                  <Image src={item.image} alt={item.name} fill className="object-cover" />
                </div>

                {/* Details */}
                <div className="flex-1 min-w-0">
                  <p className="text-xs tracking-[0.15em] text-stone-400 uppercase mb-1">
                    {item.category} Shirt
                  </p>
                  <h3
                    className="text-stone-800 font-light leading-snug"
                  ...
                  <div className="mt-2 flex items-center gap-2">
                    <label className="text-xs text-stone-400 tracking-wide">Size:</label>
                    <select
                      value={item.selectedSize}
                      onChange={(e) => updateSize(item.id, item.selectedSize, e.target.value)}
                      className="text-xs text-stone-700 border border-stone-200 bg-white px-2 py-1 focus:outline-none focus:border-stone-500 transition-colors cursor-pointer hover:border-stone-400"
                    >
                      {item.sizes.map((s) => (
                  ...
                  <div className="flex justify-between items-end mt-4">
                    ...
                    <div className="flex items-center gap-0 border border-stone-200">
                      <button
                        ...
                        className="w-8 h-8 flex items-center justify-center text-stone-500 hover:bg-stone-100 hover:text-red-500 transition-colors text-base"
                      >
                        {item.quantity <= 1 ? (
                      ...
                      <span className="w-8 h-8 flex items-center justify-center text-xs font-medium text-stone-700 border-x border-stone-200">
                        {item.quantity}
                      </span>
                      <button
                        ...
                        className="w-8 h-8 flex items-center justify-center text-stone-500 hover:bg-stone-100 transition-colors text-base disabled:opacity-30 disabled:cursor-not-allowed disabled:hover:bg-transparent"
                      >
                        +
                      </button>
                    </div>
                    ...
                    <div className="flex flex-col items-end gap-1">
                      <p className="text-stone-700 text-sm font-medium">
                        ...
                        {item.quantity > 1 && (
                          <span className="text-stone-400 font-light ml-1 text-xs">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white border border-stone-100 p-8 sticky top-28">
              ...
              <div className="space-y-3 text-sm mb-6">
                ...
                <div className="flex justify-between text-stone-500">
                  <span>Delivery</span>
                  <span className={getDeliveryCharge(totalPrice, deliveryCharge, freeThreshold) === 0 ? "text-emerald-600" : ""}>
                    {getDeliveryCharge(totalPrice, deliveryCharge, freeThreshold) === 0 ? "Free" : `₹${deliveryCharge}`}
                  </span>
                </div>
                {freeThreshold > 0 && totalPrice < freeThreshold && (
                  <p className="text-xs text-stone-400 font-light">
                    Add ₹{(freeThreshold - totalPrice).toLocaleString()} more for free delivery
                  </p>
                )}
              </div>

              <div className="border-t border-stone-100 pt-5 mb-6 flex justify-between items-center">
                <span className="text-lg text-stone-900" style={{ fontFamily: "'Cormorant Garamond', serif" }}>
                  Total
                </span>
              ...
              {stockIssues.length > 0 ? (
                <button
                  disabled
                  className="block w-full text-center bg-stone-300 text-stone-500 py-4 text-xs tracking-[0.3em] uppercase font-medium cursor-not-allowed"
                ...
                <button
                  ...
                  className="flex w-full items-center justify-center bg-stone-900 text-white py-4 text-xs tracking-[0.3em] uppercase font-medium hover:bg-stone-700 transition-all duration-300 disabled:bg-stone-400 disabled:cursor-not-allowed"
                >
                  {checkingOut ? (
                    <span
                      aria-label="Loading"
